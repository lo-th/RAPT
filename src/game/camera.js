...
RAPT.Camera = function(playerA, playerB, width, height) {
	...
	if (RAPT.useBackgroundCache) {
		this.backgroundCacheA = new RAPT.BackgroundCache('a');
		this.backgroundCacheB = new RAPT.BackgroundCache('b');
	} else {
...
RAPT.Camera.prototype = {
	constructor: RAPT.Camera,
	draw : function(c, renderer) {
		...
		temp = new RAPT.Vector(this.width / Math.abs(temp.x), this.height / Math.abs(temp.y));
		...
		if(!isSplit) {
			...
			c.save();
			this.clipHelper(c, this.width / 2, this.height / 2, split);
			renderer.render(c, centerA, this.width, this.height, this.backgroundCacheA);
			c.restore();

			// draw world from b's point of view
			c.save();
			this.clipHelper(c, this.width / 2, this.height / 2, split.mul(-1));
	...
	clipHelper : function(c, w, h, split) {
		var tx = h / split.y;
		var ty = w / split.x;
		c.beginPath();
