...
RAPT.BackgroundCache = function (name) {
...
RAPT.BackgroundCache.prototype = {
	constructor: RAPT.BackgroundCache,
	draw : function(c, xmin, ymin, xmax, ymax) {
		// if cache is invalid, update cache
		if (this.modificationCount != RAPT.gameState.modificationCount || xmin < this.xmin || xmax > this.xmax || ymin < this.ymin || ymax > this.ymax) {
			this.modificationCount = RAPT.gameState.modificationCount;
			...
			var width = Math.ceil(viewportWidth * RAPT.gameScale);
			var height = Math.ceil(viewportHeight * RAPT.gameScale);
			...
			this.c.scale(RAPT.gameScale, -RAPT.gameScale);
			this.c.lineWidth = 1 / RAPT.gameScale;
			
			// render
			this.c.translate(-(this.xmin + this.xmax) / 2, -(this.ymin + this.ymax) / 2);
			RAPT.gameState.world.draw(this.c, this.xmin, this.ymin, this.xmax, this.ymax);
		...
		c.drawImage(this.canvas,
			Math.round((this.xmin - xmin) * RAPT.gameScale),
			Math.round((2 * ymin - ymax - this.ymin) * RAPT.gameScale)
		);
		c.restore();
	}
}
