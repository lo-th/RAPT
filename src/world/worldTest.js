...
RAPT.World.prototype = {
	...
	init:function(w, h, spawnPoint, goal){
		...
		var p;
		while(i--){
			p = this.findPosition(i);
			this.cells[i] = new RAPT.Cell(p.x, p.y, RAPT.CELL_SOLID);
		}
	...
	getCell : function(x, y) {
		...
		return (x >= 0 && y >= 0 && x < this.width && y < this.height) ? this.cells[this.findId(x,y)] : null;
	},
	...
	getCellType : function(x, y) {
		...
		return (x >= 0 && y >= 0 && x < this.width && y < this.height) ? this.cells[this.findId(x,y)].type : RAPT.CELL_SOLID;
	},
	getEdges : function(x, y) {
		//return (x >= 0 && y >= 0 && x < this.width && y < this.height) ? this.cells[x][y].edges : [];
		return (x >= 0 && y >= 0 && x < this.width && y < this.height) ? this.cells[this.findId(x,y)].edges : [];
	},
	setCell : function(x, y, type) {
		//this.cells[x][y] = new RAPT.Cell(x, y, type);
		this.cells[this.findId(x,y)] = new RAPT.Cell(x, y, type);
	},
	createAllEdges : function() {
		...
		//	for (var y = 0; y < this.cells[0].length; y++) {
				...
				this.cells[i].edges = this.createEdges(p.x, p.y);
				this.totalEdge+=this.cells[i].edges.length;
			//}
		}
	},
	...
	getEdgesInAabb : function(aabb, color) {
		...
		for(var x = xmin; x < xmax; x++)
			for(var y = ymin; y < ymax; y++)
				edges = edges.concat(this.cells[this.findId(x,y)].getBlockingEdges(color));
	...
	getCellsInAabb : function(aabb) {
		...
		for(var x = xmin; x < xmax; x++)
			for(var y = ymin; y < ymax; y++)
				cells = cells.concat(this.cells[this.findId(x,y)]);
