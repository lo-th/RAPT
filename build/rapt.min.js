...
this.info[0]=a.x||0;this.info[1]=a.y||0;this.info[2]=this.state;return this.info}};RAPT.W3D=null;RAPT.MAT_PLAYER=null;RAPT.GEO={};var debug=debug||null;RAPT.Loader=function(a,b){var c=new THREE.SEA3D({});c.onComplete=function(a){a=c.meshes.length;for(var e;a--;)e=c.meshes[a],RAPT.GEO[e.name]=e.geometry;b()};c.load(a)};
RAPT.World3D=function(a){this.isMobile=!1;this.antialias=!0;var b=navigator.userAgent;if(b.match(/Android/i)||b.match(/webOS/i)||b.match(/iPhone/i)||b.match(/iPad/i)||b.match(/iPod/i)||b.match(/BlackBerry/i)||b.match(/Windows Phone/i))this.isMobile=!0,this.antialias=!1;this.debug=document.getElementById("debug");this.vs={w:window.innerWidth,h:window.innerHeight,r:0.0084,mw:0,mh:0,d:10,fov:60};this.is2D=this.split=!1;this.camera=[];this.initCameras();this.scene=new THREE.Scene;RAPT.GEO.front=this.plane("front");
RAPT.GEO.back=this.plane("back");RAPT.GEO.floor=this.plane("floor",-Math.PI/2,-0.5,!1,0,0.5);RAPT.GEO.dfloor=this.plane("floor",-Math.PI/2,0,!0,Math.PI/4,0.5);b=[];b[0]=THREE.ImageUtils.loadTexture("textures/level.png");b[1]=THREE.ImageUtils.loadTexture("textures/particle.png");b[2]=THREE.ImageUtils.loadTexture("textures/player.png");b[3]=THREE.ImageUtils.loadTexture("textures/enemy.png");b[4]=THREE.ImageUtils.loadTexture("textures/level.png");b[4].flipY=!1;for(var c=b.length;c--;);RAPT.MAT_LEVEL=
new THREE.ShaderMaterial(RAPT.MakeBasicShader({map:b[0],transparent:!0}));RAPT.MAT_DOOR_R=new THREE.ShaderMaterial(RAPT.MakeBasicShader({map:b[4],color:16711680,transparent:!0}));RAPT.MAT_DOOR_B=new THREE.ShaderMaterial(RAPT.MakeBasicShader({map:b[4],color:26367,transparent:!0}));RAPT.MAT_DOOR=new THREE.ShaderMaterial(RAPT.MakeBasicShader({map:b[4],color:16777215,transparent:!0}));RAPT.MAT_PLAYER=new THREE.ShaderMaterial(RAPT.MakeBasicShader({map:b[2],transparent:!0,side:THREE.DoubleSide,alphaTest:0.1}));
RAPT.MAT_ENEMY=new THREE.ShaderMaterial(RAPT.MakeBasicShader({map:b[3],transparent:!0,side:THREE.DoubleSide,alphaTest:0.1}));RAPT.Particle.init3d(this.scene,b[1]);this.doors=[];this.sprites=[];this.renderer=new THREE.WebGLRenderer({precision:"mediump",canvas:a,antialias:this.antialias,alpha:!1});this.renderer.setPixelRatio(window.devicePixelRatio);this.renderer.setSize(this.vs.w,this.vs.h);this.renderer.setClearColor(7566195,1);this.effect=new RAPT.SplitEffect(this.renderer);this.effect.setSize(this.vs.w,
this.vs.h);RAPT.W3D=this;a.onmousewheel=function(a){RAPT.W3D.onMouseWheel(a)}};
